...
const registrationForm = document.getElementById("doctor_registration-form");
...
registrationForm.addEventListener("submit", async (event) => {
  ...
  const reg_numberRegex = /^[A-Z]\d{7}$/;


  showLoadingSpinner();
  
  // Basic client-side validation
  if (fullname == "" || email == "" || contact == "" || reg_number == "" || password == "" || confirmPassword == "") {
  ...
  if (!reg_numberRegex.test(reg_number)) {
    displayFloatingCard('Invalid registration number format.', 'error');
    hideLoadingSpinner();
    return;
  }
  ...
  // Send data to server
  try {
    const response = await fetch('/doctor_registration', {
      method: 'POST',
      headers: {
