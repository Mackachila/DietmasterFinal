...
document.getElementById("new-report-form").addEventListener("submit", async function (event) {
    ...
    const email = document.getElementById("patient-select").value;
    ...
    try {
        ...
        if (!response.ok) {
            messageElement.style.background = 'rgb(241, 222, 222)'; // Red background for error
            messageElement.style.color = 'red';  // Red text for error
            ...
            messageElement.style.background = '#e8f5e9'; // Green background for success
            messageElement.style.color = '#2e7d32'; // Green text for success
            messageElement.style.display = 'block';
            messageElement.textContent = data.message;
            ...
            // Reset the form and reload reports
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchSentReports(page) {
        try {
            const response = await fetch(`/fetch-sent-reports?page=${page}&limit=${reportsPerPage}`);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function searchReports(query) {
        try {
            ...
            const response = await fetch(`/fetch-sent-reports?page=1&limit=1000`);
            const data = await response.json();
