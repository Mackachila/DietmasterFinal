...
router.get('/get-user', (req, res) => {
  ...
  const query = 'SELECT username, email, contact FROM dietmaster_members WHERE email = ?';
...
router.post('/update-details', async (req, res) => {
  ...
  router.post('/userregistration', async (req, res) => {
    ...
    try {
      ...
      const insertQuery = `INSERT INTO dietmaster_members (username, email, contact, password) VALUES (?, ?, ?, ?)`;
      await pool.promise().execute(insertQuery, [fullname, email, contact, hashedPassword]);
      // Insert the user into the database only if email sending succeeds
